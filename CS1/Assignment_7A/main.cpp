...
Purpose: The purpose of this program is to find the total cost of all the items brought to the farmer's market.
*/
...
int main()
{
    ...
    else
    {
        ...
        // Putting the arrays in alphabetical order and making sure the rest of the arrays still correlate
        sortArraysInAlphabeticalOrder(pricesForProducts, namesForProducts, numberOfItems);
...
void sortArraysInAlphabeticalOrder(float (&pricesForProducts)[28], string (&namesForProducts)[28], int (&numberOfItems)[28])
{
    ...
    while (!loopFlag)
    {
        ...
        for (int x = 0; x < 27; ++x)
        {
            //if (namesForProducts[x].compare(namesForProducts[x + 1]) < 0)
            if (namesForProducts[x + 1].compare(namesForProducts[x]) < 0)
            {
                // Keeping the loop going, as not all of the elements are in correct order
                loopFlag = false;

                // Swapping the string array
                stringSwap(namesForProducts, x);

                // Swapping the int array
                intSwap(numberOfItems, x);

                // Swapping the float array
                floatSwap(pricesForProducts, x);
            }
        }
    }
}
